...
const INDUSTRY_MODULE_REGISTRY: Partial<Record<IndustryKey, IndustryModule>> = {};
const BUSINESS_TYPE_TO_INDUSTRY: Partial<Record<BusinessType, IndustryKey>> = {};

function register(module: IndustryModule) {
...
export function registerIndustryModule(module: IndustryModule) {
  return register(module);
}

export function getIndustryModule(key?: string | null) {
  if (!key) return null;
  return isIndustryKey(key) ? INDUSTRY_MODULE_REGISTRY[key] ?? null : null;
}

export function listIndustryModules() {
  return INDUSTRY_KEYS.map((key) => INDUSTRY_MODULE_REGISTRY[key]).filter((module): module is IndustryModule => Boolean(module));
}

export function resolveIndustryKey(input: { industryKey?: string | null | undefined; businessType?: string | null | undefined; fallback?: IndustryKey } = {}) {
...
export function resolveIndustryModule(input: { industryKey?: string | null | undefined; businessType?: string | null | undefined; fallback?: IndustryKey } = {}) {
...
function typeOption(value: BusinessType, label: string, description: string): IndustryBusinessTypeOption {
  return { value, label, description };
}

function dashboardWidget(widget: DashboardWidget) {
  return widget;
}
